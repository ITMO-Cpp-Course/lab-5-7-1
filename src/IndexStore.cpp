...
UpdateTransaction::UpdateTransaction(IndexStore& store)
    : store_(&store), snapshot_(store.index_) // копируем текущее состояние
      ,
      committed_(false)
{
}
...
{
    // Если commit() не был вызван — ничего не делаем:
    // snapshot_ просто уничтожается, оригинал не тронут.
}
    ...
    : store_(other.store_), snapshot_(std::move(other.snapshot_)), committed_(other.committed_)
{
    ...
    other.committed_ = true; // предотвращаем двойной откат
}
...
Result<> UpdateTransaction::commit()
{
    ...
    // Атомарно заменяем индекс в хранилище на рабочую копию
